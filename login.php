<?php
  ...
  if(isset($_POST['send'])){
    ...
    $pass=md5($contra);
    $consulta="SELECT ESTADO_ID FROM USUARIO WHERE EMAIL='".$user."' AND PASSWORD='".$pass."'";
    $exec=db_query($consulta);
    $resultado=db_fetch_array($exec);
    if(is_array($resultado)){
      echo "<script>alert('Bienvenido')</script>";
      if($resultado[0]==1){
        $nuevaURL="index.php?variable=".$user;
        
        header("Location: ".$nuevaURL."");
        exit();
      }else{
        echo "<script>alert('Error usuario no activo')</script>";
      }
    }else{
...
?>                    
  <div class="page-heading">
    <div class="container">
     ...
     <h2>Inicia o Crea una Cuenta</h2>
  </div>
        </div>
        <!--col-xs-12-->
      </div>
        ...
        <fieldset class="col2-set">
            <div class="col-1 new-users"> 
                ...
                <div class="content">
                    ...
                    <p>Si eres nuevo crea una cuenta dando click en el boton a continuacion</p>
                     <div class="buttons-set">
                    <button type="button" title="Create an Account" class="button create-account" onClick=""><span><span><a href="register.php" style="color: white;">Crea una Cuenta</a></span></span></button>
                </div>
                </div>
            </div>
                    ...
                    <p>Si ya tienes una cuenta inicia sesion.</p>
                      ...
                      <ul class="form-list">
                        <li>
                            ...
                            <div class="input-box">
                                <input type="email" name="email" id="email" class="input-text required-entry validate-email" title="Correo Electronico" required/>
                            </div>
                        </li>
                        <li>
                            <label for="pass">Contraseña<em class="required">*</em></label>
                            <div class="input-box">
                                <input type="password" name="password" class="input-text required-entry validate-password" id="pass" title="contraseña" required/>
                            </div>
                        </li>
                      </ul>
...
<p class="required">* Campos Requeridos</p>
                    ...
                    <button type="submit" class="button login" title="Login" name="send" id="send2"><span>Iniciar Sesion</span></button>
